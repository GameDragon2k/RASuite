...
LRESULT CALLBACK Main_Proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		if (LOWORD(wParam) >= IDM_RA_FILES_ACHIEVEMENTS &&
				LOWORD(wParam) < IDM_RA_PARSERICHPRESENCE)
			{
...
void SetupMenu ( HWND hWnd ) {
	...
	AppendMenu( hMenu, MF_POPUP|MF_STRING, (UINT_PTR)RA_CreatePopupMenu(), TEXT("&RetroAchievements") );
